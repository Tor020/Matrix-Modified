    ...
    //1 = y co-ordinate of the drop(same for every drop initially)
        for(var x = 0; x < columns; x++)
                letSouth[x] = 1; //this value refers to where on the screen the characters start their initial descent. the 1 basically puts it 1 section from the very top of the page. a -1 would make it start above the page
        ...
        function draw()
        {
            ...
            for(var i = 0; i < letSouth.length; i++)
            {
                ...
                if(letSouth[i]*font_size > c.height && Math.random() > 0.975)
                    letSouth[i] = 0;
